    ...
    <>
      <section className="relative overflow-hidden bg-[#081a30]">
        <div className="absolute inset-0 -z-10">
          <Image src={p.image} alt={`Estimation et rachat de bijoux et diamants à Courchevel ${p.cityName}`} fill priority sizes="100vw" className="object-cover opacity-35" />
        ...
        <div className="container-x pt-24 pb-16 md:pt-32">
          ...
          <div className="grid lg:grid-cols-12 gap-10 items-start text-ink-50">
            <div className="lg:col-span-7">
              ...
              <h1 className="h1 mt-4 text-white">Rachat de Bijoux & Estimation à Courchevel — {p.cityName}</h1>
              <p className="lead mt-5 text-ink-200">
                Vous séjournez ou résidez à {p.cityName}, au cœur des 3 Vallées ou en station alpine et souhaitez vendre des diamants certifiés, bijoux d'époque, haute joaillerie signée ou montres de collection ? Nos gemmologues experts vous reçoivent en toute discrétion dans nos salons privés à Courchevel 1850 ou effectuent un déplacement sécurisé directement dans votre chalet privé ou palace.
              </p>
            ...
            <div className="lg:col-span-5">
              <div className="rounded-2xl bg-white p-6 md:p-8 shadow-2xl">
                <ContactForm
                  variant="estimation"
                  category={`Courchevel — ${p.cityName}`}
                  defaultSubject={`Demande d'estimation depuis ${p.cityName}`}
                  source={`local:${p.slug}`}
                  headline="Estimation Confidentielle"
                  intro="Transmettez-nous les détails de vos joyaux pour une première valorisation sous 24 heures."
                />
              </div>
            </div>
          </div>
      ...
      <Section>
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-7 space-y-6 text-ink-700 leading-relaxed">
            <SectionHeading
              eyebrow="Expertise Courchevel & 3 Vallées"
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="rounded-2xl border border-ink-900/10 bg-ink-50/50 p-6 md:p-8">
              <h3 className="font-display text-xl text-ink-900 mb-4">Paiement Immédiat & Discrétion Alpine</h3>
      ...
      <Section className="bg-ink-50">
        <div className="max-w-4xl mx-auto text-center space-y-4">
          <h2 className="font-display text-2xl md:text-3xl text-ink-900 font-semibold">
            Prendre Rendez-vous à Courchevel ({p.cityName})
          </h2>
          ...
          <div className="pt-4 flex flex-wrap justify-center gap-4">
            <Link href="/estimation-gratuite" className="btn-gold">Demander une Estimation</Link>
            <Link href="/contact" className="btn-outline">Nos Salons Privés 1850</Link>
          </div>
        </div>
      </Section>
