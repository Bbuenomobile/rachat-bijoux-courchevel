...
export function LocalPage(p: LocalPageProps) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: `Rachat Bijoux à ${p.district}`, item: `${site.url}/${p.slug}` },
    ],
  };
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="pt-2 flex flex-wrap gap-4">
                <Link href="/estimation-gratuite" className="btn-gold py-3 px-8 text-xs uppercase tracking-wider font-semibold">
                  Prendre Rendez-vous à {p.district}
                </Link>
                <a
      ...
      <Section>
        <div className="container-x max-w-4xl space-y-8">
          <SectionHeading
            ...
            title={`FAQ pour les Résidents et Visiteurs de ${p.district}`}
            align="center"
          />
      ...
      <Section className="bg-sand-50">
        <div className="container-x max-w-4xl">
          <div className="bg-white rounded-2xl border border-ink-900/10 p-8 md:p-12 shadow-sm">
            <ContactForm
              headline={`Demande d'Expertise Confidentielle à ${p.district}`}
              source={`local:${p.slug}`}
            />
          </div>
        </div>
